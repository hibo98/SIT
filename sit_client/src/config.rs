...
impl Config {
    pub fn setup() -> Result<()> {
        let software = RegKey::predef(HKEY_LOCAL_MACHINE).open_subkey_with_flags("SOFTWARE", KEY_ALL_ACCESS)?;
    ...
    pub fn get_web_api() -> Result<String> {
        let settings = RegKey::predef(HKEY_LOCAL_MACHINE).open_subkey("SOFTWARE\\SCHKOLA gGmbH\\S-IT Client\\Settings")?;
        Ok(settings.get_value("web_api")?)
    }

    pub fn get_uuid() -> Result<Option<Uuid>> {
        let client_info = RegKey::predef(HKEY_LOCAL_MACHINE).open_subkey("SOFTWARE\\SCHKOLA gGmbH\\S-IT Client\\Client Info")?;
    ...
    pub fn set_uuid(uuid: Uuid) -> Result<()> {
        let client_info = RegKey::predef(HKEY_LOCAL_MACHINE).open_subkey_with_flags("SOFTWARE\\SCHKOLA gGmbH\\S-IT Client\\Client Info", KEY_WRITE)?;
